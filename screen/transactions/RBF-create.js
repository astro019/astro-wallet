...
export default class SendCreate extends Component {
  ...
  async componentDidMount() {
    ...
    let txMetadata = NasdaApp.tx_metadata[this.state.txid];
    if (txMetadata) {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        NasdaApp.tx_metadata[this.state.txid] = txMetadata || {};
        NasdaApp.tx_metadata[this.state.txid]['last_sequence'] = lastSequence;
        ...
        NasdaApp.tx_metadata[txid] = NasdaApp.tx_metadata[this.state.txid];
        NasdaApp.tx_metadata[txid]['txhex'] = tx;
        //
        NasdaApp.saveToDisk();
        console.log(
          'NasdaApp.txMetadata[this.state.txid]',
          NasdaApp.tx_metadata[this.state.txid],
        );
      } catch (err) {
        console.log(err);
        return this.setState({
